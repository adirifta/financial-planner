    ...
    if(metaThemeColor){
        switch (setting) {
            ...
            case 'light':
                ...
                break;
            case 'system':
                window.matchMedia('(prefers-color-schema: dark)').matches ? metaThemeColor.setAttribute('content', '#000000') : metaThemeColor.setAttribute('content', '#ffffff');
                break;
            default:
                metaThemeColor.setAttribute('content', '#ffffff');
                break;
...
function updateTheme(){
    ...
    if(currentTheme === 'system'){
         window.matchMedia('(prefers-color-schema: dark)').matches ? document.documentElement.classList.add('dark') : document.documentElement.classList.add('light');
    } else {
